    ...
    <div class="card mb-5 mb-xl-10">
        ...
        <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
            ...
            <div class="card-title m-0">
                <h3 class="fw-bold m-0">Ubah pelatihan</h3>
            </div>
            <!--end::Card title-->
        </div>
        <!--begin::Card header-->
        ...
        <div id="kt_account_settings_profile_details" class="collapse show">
            <!--begin::Form-->
            <form id="kt_account_profile_details_form" class="form" action="{{ route('courses.update', $course->id) }}"
                method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                <input type="hidden" name="image_old" value="{{ $course->image }}">
                <!--begin::Card body-->
                <div class="card-body border-top p-9">
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="title"
                                ...
                                placeholder="judul..." value="{{ $course->title }}" required autocomplete="off" />
                    ...
                    <div class="row mb-6">
                        ...
                        <!--begin::Col-->
                        <div class="col-lg-8 fv-row">
                            <textarea class="form-control form-control-solid @error('description') is-invalid @enderror" rows="8"
                                name="description" required autocomplete="off" placeholder="deskripsi...">{!! $course->description !!}</textarea>
                            <div class="invalid-feedback">
                                @error('description')
                                    {{ $message }}
                                @enderror
                            ...
                            <select name="id_category" data-control="select2" data-placeholder="pilih kategori..."
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $course->id_category === $category->id ? 'selected' : '' }}>{{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="invalid-feedback">
